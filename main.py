...
class BaseImage:
    __pixels: np.ndarray  # tensor przechowujacy piksele obrazu
    ...
    def __init__(self, path: Any, colorModel: ColorModel) -> None:
        if isinstance(path, str):
            self.__pixels = imread(path)
        else:
            self.__pixels = path
    ...
    def showLayer(self, layer_id: int) -> None:
        layer = self.__pixels[:, :, layer_id]
        imshow(layer)
        matplotlib.pyplot.show()

    "metoda wyswietlajaca obraz RGB dzieląc go na 3 warstwy R | G | B"
    def showAsRgbLayers(self) -> None:
        redLayer, greenLayer, blueLayer = np.squeeze(np.dsplit(self.__pixels, self.__pixels.shape[-1]))
    ...
    def showHsvConversions(self) -> None:
        ...
        axis[0].imshow(self.__pixels)
        axis[0].set_title("RGB")
        self.fromRgbToHsv()
        axis[1].imshow(self.__pixels)
        axis[1].set_title("RGB -> HSV")
        self.fromHsvToRgb()
        axis[2].imshow(self.__pixels)
